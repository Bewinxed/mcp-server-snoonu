...
class SnoonuAutomation {
	...
	private async createContextWithSession() {
		...
		if (sessionExists) {
					...
					origins: session.localStorage ? [
						{
							...
							localStorage: Object.entries(session.localStorage).map(
			...
			console.log(`✅ Session restored with ${session.cookies.length} cookies`);
			const authCookie = session.cookies.find(c => c.name === "authToken");
			if (authCookie) {
				console.log("🔑 Auth token restored:", authCookie.value.substring(0, 10) + "...");
	...
	private async saveSession(force: boolean = false) {
		...
		try {
			...
			console.log(`💾 Session saved (${cookies.length} cookies, authToken: ${!!authCookie})`);
		} catch (error) {
	...
	private async handleLoginFlow() {
		...
		try {
			...
			if (authCookie) {
				console.log("🔑 Auth token captured:", authCookie.value.substring(0, 10) + "...");
			} else {
	...
	async verifySession() {
		...
		if (loggedIn) {
			const cookies = await this.context!.cookies();
			const authCookie = cookies.find(c => c.name === "authToken");
			console.log("🍪 Auth token:", authCookie?.value?.substring(0, 20) + "...");
			
			// Navigate to a page that requires auth to verify the session works
			await this.page.goto("https://snoonu.com/en/profile", { waitUntil: "networkidle" });
			
			// Check if we get redirected to login
			const url = this.page.url();
			if (url.includes("profile")) {
				console.log("✅ Session is valid - able to access profile page");
				return true;
			} else {
				console.log("⚠️ Session invalid - redirected away from profile");
				return false;
			}
		}
